...
from models.qa.utils import find_answer_sentence
...
class LogitRanker(Component):
    ...
    def __call__(self, contexts_batch, questions_batch, doc_ids_batch, doc_pages_batch=None):
        ...
        batch_best_answers_logit = []
        batch_best_answers_place = []
        batch_best_answers_doc_ids = []
        batch_best_answers_doc_pages = []
        ...
        for quest_ind, [contexts, questions] in enumerate(zip(contexts_batch, questions_batch)):
            ...
            if self.sort_noans:
                results_sort = sorted(results, key=lambda x: (x[0] != '', x[2], x[3]), reverse=True)
            else:
                results_sort = sorted(results, key=lambda x: (x[2], x[3]), reverse=True)
            best_answers = [x[0] for x in results_sort]
            best_answers_place = [x[1] for x in results_sort]
            best_answers_score = [x[2] for x in results_sort]
            best_answers_logit = [x[3] for x in results_sort]
            best_answers_contexts = [x[4] for x in results_sort]
            batch_best_answers.append(best_answers[:self.top_n])
            batch_best_answers_place.append(best_answers_place[:self.top_n])
            batch_best_answers_score.append(best_answers_score[:self.top_n])
            batch_best_answers_logit.append(best_answers_logit[:self.top_n])
            ...
            batch_best_answers_sentences.append(best_answers_sentences[:self.top_n])
            ...
            if doc_pages_batch is not None:
                doc_ind = [results.index(x) for x in results_sort]
                batch_best_answers_doc_pages.append(
                    [doc_pages_batch[quest_ind][i] for i in doc_ind][:len(batch_best_answers[-1])])
        ...
        if self.top_n == 1:
            ...
            batch_best_answers_logit = [x[0] for x in batch_best_answers_logit]
            batch_best_answers_doc_ids = [x[0] for x in batch_best_answers_doc_ids]
            batch_best_answers_doc_pages = [x[0] for x in batch_best_answers_doc_pages]
            batch_best_answers_sentences = [x[0] for x in batch_best_answers_sentences]

        if self.return_answer_sentence:
            return batch_best_answers, batch_best_answers_score, batch_best_answers_logit, batch_best_answers_place, \
                   batch_best_answers_doc_ids, batch_best_answers_doc_pages, batch_best_answers_sentences
        return batch_best_answers, batch_best_answers_score, batch_best_answers_logit, batch_best_answers_place, \
            batch_best_answers_doc_ids
