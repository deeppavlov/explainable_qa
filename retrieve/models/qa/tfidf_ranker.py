...
class TfidfRanker(Component):
    ...
    def __init__(self, vectorizer: HashingTfIdfVectorizer, top_n=5, out_top_n = 5, active: bool = True,
                       thres: int = 60, filter_flag: bool = False, **kwargs):
    ...
    def __call__(self, questions: List[str],
        ...
        for question, q_tfidf, entity_substr_list, tags_list in \
            ...
            doc_ids_scores = list(zip(doc_ids, doc_scores))
            doc_ids_scores = [(doc_id, score) for doc_id, score in doc_ids_scores if score > self.thres]
            doc_ids = [elem[0] for elem in doc_ids_scores]
            doc_scores = [elem[1] for elem in doc_ids_scores]
